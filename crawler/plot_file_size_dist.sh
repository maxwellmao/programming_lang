...
do
    ...
    echo $repos_path
    for commit in 'latest' 'initial'
    do
        echo $commit
        awk '{split($2, a, ":");print a[1]}' ${repos_path}/${commit}_commit | sort -n | uniq -c | awk '{print $2, $1}' > ${repos_path}/${commit}_commit_all_token
        awk '{split($2, a, ":");print a[2]}' ${repos_path}/${commit}_commit | sort -n | uniq -c | awk '{print $2, $1}' > ${repos_path}/${commit}_commit_program_token
        awk '{print $3}' ${repos_path}/${commit}_commit | sort -n | uniq -c | awk '{print $2, $1}' > ${repos_path}/${commit}_commit_size
...
##
        python ../turn_to_ccdf.py ${repos_path}/${commit}_commit_all_token ${repos_path}/${commit}_commit_all_token_ccdf
        python ../turn_to_ccdf.py ${repos_path}/${commit}_commit_program_token ${repos_path}/${commit}_commit_program_token_ccdf
        python ../turn_to_ccdf.py ${repos_path}/${commit}_commit_size ${repos_path}/${commit}_commit_size_ccdf

    done

    python ../plot_dist.py ${repos_path}/initial_commit_size Initial ${repos_path}/latest_commit_size latest ${repos_path}/FileSize_xy_pdf
    python ../plot_dist.py ${repos_path}/initial_commit_size_ccdf Initial ${repos_path}/latest_commit_size_ccdf latest ${repos_path}/FileSize_xy_ccdf
...
##########################################################################################################################
#    python ../plot_ccdf.py ${repos_path}/${commit}_commit_all_token_ccdf all ${repos_path}/${commit}_commit_program_token_ccdf program ${repos_path}/${commit}_commit_size_ccdf size ${repos_path}/size_ccdf
#
